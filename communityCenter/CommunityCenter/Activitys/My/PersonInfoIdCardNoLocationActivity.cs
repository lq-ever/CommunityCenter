...
namespace CommunityCenter.Activitys.My
{
	...
	public class PersonInfoIdCardNoLocationActivity : Activity
	{
		private EditText edit_idCardLocation;
		...
		protected override void OnCreate (Bundle bundle)
		{
			...
			SetContentView(Resource.Layout.PersonInfoIdCardNoLocationLayout);
		...
		private void Save()
		{
			var idCardLocation = edit_idCardLocation.Text;
			if (string.IsNullOrEmpty (idCardLocation)) {
				Toast.MakeText(this,"身份证所在地不能为空,请输入",ToastLength.Short).Show();
				return ;
			}
							...
							RunOnUiThread(()=>
								{
									...
									Global.MyInfo.IDAddress = idCardLocation;
									ProgressDialogUtil.StopProgressDialog();
									this.Finish();
									OverridePendingTransition(Android.Resource.Animation.SlideInLeft,Android.Resource.Animation.SlideOutRight);
								});
