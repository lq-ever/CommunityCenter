	...
	public partial class MKOLUpdateElement : global::Java.Lang.Object {
		...
		public int CityID {
			get {
				...
				return JNIEnv.GetIntField (Handle, cityID_jfieldId);
			}
			set {
				if (cityID_jfieldId == IntPtr.Zero)
					cityID_jfieldId = JNIEnv.GetFieldID (class_ref, "cityID", "I");
				try {
					JNIEnv.SetField (Handle, cityID_jfieldId, value);
				} finally {
				}
			}
		}
		...
		public string CityName {
			get {
				...
				IntPtr __ret = JNIEnv.GetObjectField (Handle, cityName_jfieldId);
			...
			set {
				...
				try {
					JNIEnv.SetField (Handle, cityName_jfieldId, native_value);
				} finally {
		...
		// Metadata.xml XPath field reference: path="/api/package[@name='com.baidu.mapapi.map.offline']/class[@name='MKOLUpdateElement']/field[@name='eOLDSMissData']"
		[Register ("eOLDSMissData")]
		public const int EOLDSMissData = (int) 9;
		...
		public global::Com.Baidu.Mapapi.Model.LatLng GeoPt {
			get {
				...
				IntPtr __ret = JNIEnv.GetObjectField (Handle, geoPt_jfieldId);
			...
			set {
				...
				try {
					JNIEnv.SetField (Handle, geoPt_jfieldId, native_value);
				} finally {
					JNIEnv.DeleteLocalRef (native_value);
				}
			}
		...
		public int Ratio {
			get {
				...
				return JNIEnv.GetIntField (Handle, ratio_jfieldId);
			}
			set {
				if (ratio_jfieldId == IntPtr.Zero)
					ratio_jfieldId = JNIEnv.GetFieldID (class_ref, "ratio", "I");
				try {
					JNIEnv.SetField (Handle, ratio_jfieldId, value);
				} finally {
				}
			}
		}
		...
		public int Serversize {
			get {
				...
				return JNIEnv.GetIntField (Handle, serversize_jfieldId);
			}
			set {
				if (serversize_jfieldId == IntPtr.Zero)
					serversize_jfieldId = JNIEnv.GetFieldID (class_ref, "serversize", "I");
				try {
					JNIEnv.SetField (Handle, serversize_jfieldId, value);
				} finally {
				}
			}
		}
		...
		public bool Update {
			get {
				...
				return JNIEnv.GetBooleanField (Handle, update_jfieldId);
			}
			set {
				if (update_jfieldId == IntPtr.Zero)
					update_jfieldId = JNIEnv.GetFieldID (class_ref, "update", "Z");
				try {
					JNIEnv.SetField (Handle, update_jfieldId, value);
				} finally {
				}
			}
		}
			...
			: base (IntPtr.Zero, JniHandleOwnership.DoNotTransfer)
		{
			...
			try {
				if (GetType () != typeof (MKOLUpdateElement)) {
					SetHandle (
